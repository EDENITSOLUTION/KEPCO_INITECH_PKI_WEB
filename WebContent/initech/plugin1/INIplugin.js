...
var TimeURL = "http://" + window.location.host + "/initech/plugin1/tools/Time.jsp";
//var TimeURL = "http://" + window.location.host + "/initech/plugin/tools/Random.jsp";

var LogoURL = 'http://' + window.location.host + '/initech/plugin1/site/img/plugin.initech.com.gif';
//LogoURL = "https://devoib.shinhan.com/initech/plugin/img/banking_plugin60_logo.gif";
// LogoURL = "http://demo.initech.com/initech/plugin/banking_plugin60_logo.gif"

var E2ERandomURL = "http://" + window.location.host + "/initech/plugin1/tools/E2E_Random.jsp";

if (navigator.systemLanguage != "ko" && navigator.systemLanguage != "ko-KR")  //한글윈도우가 아닐경우
	//LogoURL = 'http://' + window.location.host + '/initech/plugin/img/' + window.location.hostname + '.gif';
	LogoURL = 'http://' + window.location.host + '/initech/plugin1/img/kepco_logo.gif';
...
var SignGateCAIP = "61.72.247.156";
...
// for flex for java
var baseURL	= "http://" + window.location.host + "/initech/plugin1/";
...
function EncForm(form) 
{
	...
	if (filetemp !=  "")
	{
		if ((form.filedata.value = obj.MakeFileData(inivf, cipher, filetemp)) == "") return false; 
	}

	eletemp = GatherValue(form, 0, true);
	if ((INIdata = obj.MakeINIpluginData(inivf, cipher, eletemp, TimeURL))=="") return false;
...
function EncForm2(form1, form2) 
{
	...
	if (filetemp !=  "") 
	{
		if ((form2.filedata.value = obj.MakeFileData(inivf, cipher, filetemp)) == "") return false; 
	}
...
function EncLink(url, encData, target, style)
{
	...
	if ((INIdata = obj.MakeINIpluginData(inivf, cipher, encData, TimeURL))=="") return false;
...
function EncFormVerify(form) 
{
	...
	if (filetemp !=  "") 
	{
		if ((form.filedata.value = obj.MakeFileData(inivfs, cipher, filetemp)) == "") return false; 
	}
	...
	eletemp = GatherValue(form, 0, true);
	if ((INIdata = obj.MakeINIpluginData(inivfs, cipher, eletemp, Random))=="") return false;
...
function EncFormVerify2(form1, form2)
{
	...
	if (filetemp !=  "") 
	{
		if ((form2.filedata.value = obj.MakeFileData(inivfs, cipher, filetemp)) == "") return false; 
	}
...
function imsi_FormVerify(form1, form2)
{
	...
	eletemp = GatherValue(form1, 0, false);
	if((form2.INIpluginData.value = obj.MakeINIpluginData(inivfs, cipher, eletemp, Random))=="") return false;
	if ((INIdata = obj.MakeINIpluginData(inivfs, cipher, eletemp, Random))=="") return false;
...
function EncLinkVerify(url, encData, target)
{
	...
	if((INIdata = obj.MakeINIpluginData(inivfs, cipher, encData, Random))=="") return;
	queryString += obj.URLEncode(INIdata);
...
function IssueCertificate_m(szCAName, szRef, szCode)
{
	...
	if (szCAName == "YESSIGN" ) { // 금결원
		...
		alert( "결과 = "+ obj.CertRequest(YessignPackage, "", Arg, challenge));
		...
		alert( "결과 = "+ obj.CertRequest(CrossCertPackage, "", Arg, challenge));
		...
		alert( "결과 = "+  obj.CertRequest(SignKoreaPackage, "", Arg, challenge));
		...
		alert( "결과 = "+  obj.CertRequest(SignGatePackage, "", Arg, challenge));
...
function EncFile(url, form) 
{
	...
	if (filetemp !=  "")
	{
		if ((form.INIfileData.value = obj.UploadEncryptFile(url, inivf, cipher, filetemp, TimeURL)) == ""){
	...
	eletemp = GatherValue(form, 0, true);
	if ((form.INIpluginData.value = obj.MakeINIpluginData(inivf, cipher, eletemp, TimeURL))=="") return false;
...
function EncFile2(url, form, form2) 
{
	...
	if ((form2.INIpluginData.value = obj.MakeINIpluginData(inivf, cipher, eletemp, TimeURL))=="") return false;
...
function EncDown(url, args) 
{
	...
	obj = ModuleInstallCheck();
	if (obj == null) return false;
	return obj.DownloadEncryptFile(url, inivf, cipher, args, TimeURL);
}
...
function EncDownVerify(url, args) 
{
	...
	obj = ModuleInstallCheck();
	if (obj == null) return false;
	return obj.DownloadEncryptFile(url, inivfs, cipher, args, TimeURL);
}
